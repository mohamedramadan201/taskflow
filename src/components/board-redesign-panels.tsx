...
type SavedView = { id: string; name: string; shared: boolean };
...
export function BoardManagePanel({ actionCenter, insights, activeRisk, onSelectRisk, onClearFocus, visibleCount, onSelectAll, savedViews, selectedViewId, onApplyView, showSaveView, onToggleSaveView, viewName, onViewNameChange, shareView, onShareViewChange, onSaveView, onDeleteView, canShareViews, onClose }: { actionCenter: ActionCenter; insights: Insights; activeRisk: ManagerActionRisk | null; onSelectRisk: (risk: ManagerActionRisk) => void; onClearFocus: () => void; visibleCount: number; onSelectAll: () => void; savedViews: SavedView[]; selectedViewId: string; onApplyView: (id: string) => void; showSaveView: boolean; onToggleSaveView: () => void; viewName: string; onViewNameChange: (value: string) => void; shareView: boolean; onShareViewChange: (value: boolean) => void; onSaveView: (event: FormEvent<HTMLFormElement>) => void; onDeleteView: () => void; canShareViews: boolean; onClose: () => void }) {
  return <PanelShell title="Manage" label="BOARD COMMANDS" onClose={onClose} className="manage-side-panel">
    <div className="board-panel-scroll">
      ...
      <section className="manage-section saved-view-panel-section"><div className="manage-section-heading"><span className="panel-section-label">Saved view</span><button type="button" className="text-button" onClick={onToggleSaveView}>{showSaveView ? "Close" : "Save current view"}</button></div><select value={selectedViewId} onChange={(event) => onApplyView(event.target.value)} aria-label="Saved task view"><option value="">All tasks</option>{savedViews.map((saved) => <option key={saved.id} value={saved.id}>{saved.shared ? "Shared · " : "My view · "}{saved.name}</option>)}</select>{selectedViewId && <button type="button" className="text-button saved-view-delete" onClick={onDeleteView}>Delete view</button>}{showSaveView && <form className="saved-view-panel-form" onSubmit={onSaveView}><input value={viewName} onChange={(event) => onViewNameChange(event.target.value)} placeholder="View name" maxLength={60} required />{canShareViews && <label><input type="checkbox" checked={shareView} onChange={(event) => onShareViewChange(event.target.checked)} /> Share with workspace</label>}<button className="primary-button small">Save view</button></form>}</section>
    </div>
  </PanelShell>;
}
...
export function BoardFiltersPanel({ filters, members, labels, teamGroups, followUpOptions, canManageWorkspace, showLabelManager, onShowLabelManager, onCreateLabel, onChange, onClear, onClose }: { filters: BoardFilterState; members: Member[]; labels: Label[]; teamGroups: TeamGroup[]; followUpOptions: string[]; canManageWorkspace: boolean; showLabelManager: boolean; onShowLabelManager: () => void; onCreateLabel: (event: FormEvent<HTMLFormElement>) => void; onChange: (key: FilterKey, value: FilterValue) => void; onClear: () => void; onClose: () => void }) {
  ...
  return <PanelShell title="Filters" label="TASK BOARD" onClose={onClose} className="filters-side-panel">
    <div className="board-panel-scroll filters-panel-scroll">
      ...
      <section className="filter-panel-grid"><NativeFilter label="Due date" value={filters.due} options={[["all", "Any due date"], ["overdue", "Overdue"], ["today", "Due today"], ["tomorrow", "Due tomorrow"], ["this-week", "Due this week"], ["next-week", "Due next week"], ["no-date", "No date"], ["custom", "Custom range"]]} onChange={(value) => onChange("due", value)} /><NativeFilter label="Recurrence" value={filters.recurrence} options={[["all", "Any recurrence"], ["recurring", "Recurring"], ["not-recurring", "Not recurring"]]} onChange={(value) => onChange("recurrence", value)} /><NativeFilter label="Team group" value={filters.teamGroupId} options={[["", "Any team group"], ["__none__", "No team group"], ...teamGroups.map((group) => [group.id, group.name] as [string, string])]} onChange={(value) => onChange("teamGroupId", value)} /><NativeFilter label="Created by" value={filters.creator} options={[["all", "All creators"], ...members.map((member) => [member.user.id, personName(member)] as [string, string])]} onChange={(value) => onChange("creator", value)} /><NativeFilter label="Follow-up person" value={filters.followUpWith} options={[["", "Any person"], ["__none__", "No person set"], ...followUpOptions.map((value) => [value, value] as [string, string])]} onChange={(value) => onChange("followUpWith", value)} /></section>
      ...
      <details className="filter-advanced-section"><summary>More filters <span aria-hidden="true">⌄</span></summary><div className="filter-panel-grid"><NativeFilter label="Completed" value={filters.completed} options={[["all", "Any time"], ["today", "Today"], ["this-week", "This week"], ["this-month", "This month"]]} onChange={(value) => onChange("completed", value)} />{labels.length > 0 && <div className="panel-filter-field"><span>Labels</span><div className="panel-label-list">{labels.map((label) => <label key={label.id}><input type="checkbox" checked={filters.labels.includes(label.id)} onChange={() => onChange("labels", toggleValue(filters.labels, label.id))} /><i className="label-dot" style={{ backgroundColor: label.color }} /> {label.name}</label>)}</div></div>}</div></details>
