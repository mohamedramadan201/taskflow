...
type OpenCommandMenu = "insights" | "manager" | "filters" | "selection" | null;
  ...
  return <>
    ...
    <div className="task-controls" ref={controlsRef}>
      <div className="board-command-row">
        ...
        {view === "board" && <div className="toolbar-menu select-visible-menu"><button type="button" className="toolbar-menu-trigger select-visible-toggle" aria-expanded={openCommandMenu === "selection"} aria-controls="select-visible-panel" onClick={() => setOpenCommandMenu((current) => current === "selection" ? null : "selection")}><span>Select visible</span><strong>{visibleTasks.length}</strong><span aria-hidden="true">⌄</span></button>{openCommandMenu === "selection" && <div id="select-visible-panel" className="toolbar-menu-panel select-visible-panel"><label><input type="checkbox" checked={allVisibleSelected} onChange={toggleAllVisibleTasks} /> Select all visible{visibleTasks.length ? ` (${visibleTasks.length})` : ""}</label>{selectedTaskIds.size > 0 && <button type="button" className="toolbar-link" onClick={() => setSelectedTaskIds(new Set())}>Clear selection</button>}</div>}</div>}
      </div>
    ...
    <div className="view-toolbar"><nav className="view-switcher" aria-label="Task view">{(["board", "list", "calendar"] as ViewMode[]).map((mode) => <button key={mode} className={view === mode ? "active" : ""} aria-pressed={view === mode} onClick={() => setView(mode)}>{mode[0].toUpperCase() + mode.slice(1)}</button>)}</nav></div>
      ...
      <aside ref={drawerRef} className="task-drawer" role="dialog" aria-modal="true" aria-labelledby="task-drawer-title" aria-busy={loadingTaskId === selected.id} tabIndex={-1} onClick={(event) => event.stopPropagation()}>
        ...
        <div className="task-detail-layout"><div className="task-detail-main"><form onSubmit={saveTask} className="detail-form">
          <fieldset disabled={!selectedEditable}>
            ...
            <details className="task-count-section drawer-details" open>
        ...
        <details className="drawer-section drawer-details" open><summary><span><strong>Activity</strong><small>{selected.activities.length} event{selected.activities.length === 1 ? "" : "s"}</small></span><span className="disclosure-chevron" aria-hidden="true">⌄</span></summary>{selected.activities.map((item) => <p className="activity-row" key={item.id}><strong>{item.actor.name || item.actor.email}</strong> {item.type.toLowerCase().replaceAll("_", " ")} <small>{new Date(item.createdAt).toLocaleString()}</small></p>)}</details>
        </div></div>
      </aside>
    </div>}
