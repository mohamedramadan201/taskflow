    ...
    <div className="task-controls" ref={controlsRef}>
      ...
      <section className="filter-toolbar" aria-label="Task filters">
        <div className={`filter-primary-row ${showMobileFilters ? "expanded" : ""}`}>
          ...
          <button type="button" className={`more-filter-button ${showMoreFilters ? "active" : ""}`} aria-expanded={showMoreFilters} onClick={() => { setShowMoreFilters((value) => !value); setShowInsights(false); setOpenFilter(null); setShowSaveView(false); }}>{moreFilterCount > 0 && <strong>{moreFilterCount}</strong>}More filters <span aria-hidden="true">⌄</span></button>
        </div>
