...
export function BoardClient({ initialTasks, members, role, canManageWorkspace: canManageWorkspaceProp, managerActionCenter, workspaceId, currentUserId, initialLabels, initialSavedViews }: { initialTasks: Task[]; members: Member[]; role: string; canManageWorkspace: boolean; managerActionCenter: ActionCenterData; workspaceId: string; currentUserId: string; initialLabels: Label[]; initialSavedViews: SavedView[] }) {
  ...
  useEffect(() => {
    function closeFloatingControls(event: PointerEvent) { if (!controlsRef.current?.contains(event.target as Node)) { setOpenFilter(null); setShowSaveView(false); } }
    function closeOnEscape(event: KeyboardEvent) { if (event.key === "Escape") { setOpenFilter(null); setShowSaveView(false); setShowLabelManager(false); setSelected(null); } }
  ...
  function clearAllFilters() { setFilters({ ...emptyFilters }); setRiskFilter(null); setSelectedViewId(""); setOpenFilter(null); }
  ...
  function applyView(id: string) { setSelectedViewId(id); const saved = savedViews.find((item) => item.id === id); setFilters(saved ? normalizeFilters(saved.filters) : { ...emptyFilters }); setOpenFilter(null); setShowSaveView(false); }
  ...
  return <>
    ...
    <section className="task-insight-strip" aria-label="Task insights"><div><strong>{tasks.length}</strong><span>Total</span></div><div><strong>{visibleTasks.length}</strong><span>Visible</span></div><div className={attentionCount ? "attention" : ""}><strong>{attentionCount}</strong><span>Needs attention</span></div><button type="button" aria-expanded={showInsights} onClick={() => { setShowInsights((value) => !value); setOpenFilter(null); }}>{showInsights ? "Hide insights" : "View insights"}<span aria-hidden="true">{showInsights ? "⌃" : "⌄"}</span></button></section>
    ...
    <div className="task-controls" ref={controlsRef}>
      ...
      <section className="filter-toolbar" aria-label="Task filters">
