...
export function BoardClient({ initialTasks, members, role, canManageWorkspace: canManageWorkspaceProp, managerActionCenter, workspaceId, currentUserId, initialLabels, initialSavedViews }: { initialTasks: Task[]; members: Member[]; role: string; canManageWorkspace: boolean; managerActionCenter: ActionCenterData; workspaceId: string; currentUserId: string; initialLabels: Label[]; initialSavedViews: SavedView[] }) {
  ...
  useEffect(() => {
    function closeFloatingControls(event: PointerEvent) { if (!controlsRef.current?.contains(event.target as Node)) { setOpenFilter(null); setShowSaveView(false); setShowFilters(false); } }
    function closeOnEscape(event: KeyboardEvent) { if (event.key === "Escape") { setOpenFilter(null); setShowSaveView(false); setShowFilters(false); setShowLabelManager(false); setSelected(null); } }
  ...
  function clearAllFilters() { setFilters({ ...emptyFilters }); setRiskFilter(null); setSelectedViewId(""); setOpenFilter(null); setShowFilters(false); }
  ...
  function applyView(id: string) { setSelectedViewId(id); const saved = savedViews.find((item) => item.id === id); setFilters(saved ? normalizeFilters(saved.filters) : { ...emptyFilters }); setOpenFilter(null); setShowSaveView(false); setShowFilters(false); }
  ...
  return <>
    ...
    <div className="task-controls" ref={controlsRef}>
      <div className="board-command-row">
        ...
        <div className="toolbar-menu"><button type="button" className="toolbar-menu-trigger" aria-expanded={showInsights} aria-controls="task-insights-panel" onClick={() => { setShowInsights((value) => !value); setShowFilters(false); setOpenFilter(null); }}><span>View insights</span><span aria-hidden="true">⌄</span></button>{showInsights && <div id="task-insights-panel" className="toolbar-menu-panel insights-panel"><div><span>Total tasks</span><strong>{tasks.length}</strong></div><div><span>Visible tasks</span><strong>{visibleTasks.length}</strong></div><div><span>Needs attention</span><strong>{attentionCount}</strong></div><div><span>Open tasks</span><strong>{compactInsights.open}</strong></div><div><span>Due in 7 days</span><strong>{compactInsights.dueSoon}</strong></div><div><span>Completed this week</span><strong>{compactInsights.completedThisWeek}</strong></div></div>}</div>
        ...
        <button type="button" className={`toolbar-menu-trigger filter-toggle ${showFilters ? "active" : ""}`} aria-expanded={showFilters} onClick={() => { setShowFilters((value) => !value); setShowInsights(false); setOpenFilter(null); setShowSaveView(false); }}><span>Filters{filterChips.length ? ` · ${filterChips.length}` : ""}</span><span aria-hidden="true">⌄</span></button>
      </div>
      ...
      {showFilters && <section className="filter-toolbar single-row" aria-label="Task filters">
