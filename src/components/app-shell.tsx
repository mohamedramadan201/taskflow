...
export function AppShell({ children, active, userName, workspaces, workspaceSlug }: { children: React.ReactNode; active: "board" | "reminders" | "emails" | "team" | "reports" | "notifications"; userName?: string | null; workspaces: WorkspaceOption[]; workspaceSlug?: string }) {
  ...
  const links: [string, string, string, NavigationIconName][] = [["board", `/board?workspace=${currentSlug}`, "Tasks", "tasks"], ["reminders", `/reminders?workspace=${currentSlug}`, "Reminder", "reminders"], ["emails", `/emails?workspace=${currentSlug}`, "Email Inbox", "email"], ["notifications", "/notifications", "Notifications", "notifications"]];
