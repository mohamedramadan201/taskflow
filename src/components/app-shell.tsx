...
export function AppShell({ children, active, userName, workspaces, workspaceSlug = "taskflow-demo" }: { children: React.ReactNode; active: "board" | "emails" | "team" | "reports" | "notifications"; userName?: string | null; workspaces: WorkspaceOption[]; workspaceSlug?: string }) {
  ...
  return <div className="app-shell"><aside className="sidebar"><Link href={`/board?workspace=${currentSlug}`} className="brand"><span>TF</span><strong>TaskFlow</strong></Link>{workspaces.length > 0 && <WorkspaceSwitcher workspaces={workspaces} currentSlug={currentSlug} />}<nav aria-label="Primary navigation">{links.map(([key, href, label]) => <Link className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined} href={href} key={key}><span>{label}</span>{key === "emails" && <EmailInboxBadge workspaceSlug={currentSlug} />}{key === "notifications" && <NotificationBadge />}</Link>)}</nav><div className="profile"><span className="avatar">{userName?.[0] || "U"}</span><div><strong>{userName || "TaskFlow user"}</strong><small>{workspaces.find((workspace) => workspace.slug === currentSlug)?.name || "Workspace"}</small></div><form action={logout}><button title="Sign out">Sign out</button></form></div></aside><main className="main-content">{children}</main><MobileNavigation links={mobileLinks} active={active} workspaceSlug={currentSlug} /></div>;
}
