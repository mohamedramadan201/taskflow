...
export function AppShell({ children, active, userName, workspaces, workspaceSlug = "taskflow-demo" }: { children: React.ReactNode; active: "board" | "emails" | "team" | "reports" | "notifications"; userName?: string | null; workspaces: WorkspaceOption[]; workspaceSlug?: string }) {
  ...
  const links = [["board", `/board?workspace=${currentSlug}`, "Tasks", "▤"], ["emails", `/emails?workspace=${currentSlug}`, "Email Inbox", "▱"], ...(isOwner ? [["team", `/team?workspace=${currentSlug}`, "Team", "♧"], ["reports", `/reports?workspace=${currentSlug}`, "Reports", "▥"]] : []), ["notifications", "/notifications", "Notifications", "♧"]] as const;
