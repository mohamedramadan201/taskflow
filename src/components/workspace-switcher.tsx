...
export function WorkspaceSwitcher({ workspaces, currentSlug }: { workspaces: WorkspaceOption[]; currentSlug: string }) {
  ...
  return <section className="workspace-switcher" aria-label="Spaces">
    <div className="space-list-header"><span className="space-list-title">Spaces</span>{canManageSpaces && <button type="button" className="space-manage-toggle" aria-expanded={managing} onClick={() => { setManaging((value) => !value); setEditingId(null); setError(""); }}>{managing ? "Done" : "Manage"}</button>}</div>
    <div className={`space-list${managing ? " managing" : ""}`} role="list">
