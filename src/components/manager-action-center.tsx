...
export function ManagerActionCenter({ actionCenter, activeRisk, expanded, onToggle, onSelectRisk }: { actionCenter: ActionCenter; activeRisk: ManagerActionRisk | null; expanded: boolean; onToggle: () => void; onSelectRisk: (risk: ManagerActionRisk) => void }) {
  return <div className={`toolbar-menu manager-action-center-menu ${expanded ? "open" : ""}`}>
    <button type="button" className="toolbar-menu-trigger" aria-expanded={expanded} aria-controls="manager-action-center-breakdown" onClick={onToggle}><span>Manager center</span><strong>{actionCenter.total}</strong><span aria-hidden="true">⌄</span></button>
