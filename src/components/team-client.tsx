...
export function TeamClient({ initialMembers, initialInvitations, initialAuditEvents, initialCustomRoles, initialTeamGroups, availablePermissions, role, currentUserId, workspaceId }: {
  ...
  const [inviteTeamGroupId, setInviteTeamGroupId] = useState("");
  ...
  async function invite(event: FormEvent) {
    ...
    const response = await fetch(`/api/workspaces/${workspaceId}/invitations`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, role: inviteRole, teamGroupId: inviteTeamGroupId || null }) });
    if (!response.ok) return setMessage((await response.json()).error);
    const created = await response.json();
    setInvitations((items) => [created, ...items.filter((item) => item.email !== created.email)]);
    setMessage(created.emailSent ? `Invitation email sent to ${email}` : created.emailQueued ? `Invitation queued for ${email}. Google Apps Script will send it shortly.` : `Invitation created for ${email}, but the email could not be sent. Check email delivery configuration.`);
    setEmail("");
    setInviteTeamGroupId("");
    await refreshAudit();
  }

  async function activateMember(userId: string) {
    setMessage("");
    const response = await fetch(`/api/workspaces/${workspaceId}/members/${userId}/activation`, { method: "POST" });
    if (!response.ok) return setMessage((await response.json()).error);
    const activated = await response.json();
    setInvitations((items) => [activated, ...items.filter((item) => item.email !== activated.email)]);
    setMessage(activated.emailSent ? `Activation email sent to ${activated.email}` : activated.emailQueued ? `Activation link queued for ${activated.email}.` : `Activation could not be sent to ${activated.email}.`);
    await refreshAudit();
  }
