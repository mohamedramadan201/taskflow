...
export async function GET(request: Request) {
  try {
    ...
    const { workspace, subject, user } = await requireWorkspaceBySlug(slug);
    ...
    const [tasks, outputTasks, members] = await Promise.all([prisma.task.findMany({ where: { AND: [visibleTasks, ...(since ? [{ createdAt: { gte: since } }] : [])] }, select: taskSelect, orderBy: [{ createdAt: "desc" }, { id: "desc" }], take: REPORT_TASK_LIMIT + 1 }), prisma.task.findMany({ where: { AND: [visibleTasks, { completedAt }] }, select: taskSelect, orderBy: [{ completedAt: "desc" }, { id: "desc" }], take: REPORT_TASK_LIMIT + 1 }), prisma.workspaceMember.findMany({ where: { workspaceId: workspace.id }, select: { id: true, weeklyCapacityMinutes: true, availability: { select: { date: true, availableMinutes: true } }, user: { select: { id: true, name: true, email: true } } } })]);
