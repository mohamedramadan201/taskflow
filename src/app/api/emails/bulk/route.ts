...
export async function POST(request: Request) {
  try {
    ...
    if (emails.length !== input.emailIds.length) throw new HttpError(404, "One or more emails could not be found");
    ...
    if (input.action === "ASSIGN") {
      ...
      const updated = await prisma.$transaction(async (tx) => {
        const result = await tx.inboundEmail.updateMany({ where: { id: { in: input.emailIds }, workspaceId, status: { not: "CONVERTED" } }, data: { handledByUserId: assigneeUserId, handledAt: null } });
    ...
    const updated = await prisma.inboundEmail.updateMany({ where: { id: { in: input.emailIds }, workspaceId, status: { not: "CONVERTED" } }, data: { status, handledAt: handled ? new Date() : null, handledByUserId: handled ? access.user.id : null } });
    return Response.json({ count: updated.count, action: input.action });
