...
export async function POST(request: Request, { params }: { params: Promise<{ emailId: string }> }) {
  try {
    const { emailId } = await params; const email = await prisma.inboundEmail.findUnique({ where: { id: emailId }, select: { id: true, workspaceId: true, status: true, subject: true } }); if (!email) throw new HttpError(404, "Email not found");
