...
export async function POST(request: Request, { params }: { params: Promise<{ emailId: string }> }) {
  try {
    const { emailId } = await params; const email = await prisma.inboundEmail.findUnique({ where: { id: emailId }, select: { id: true, workspaceId: true, status: true, subject: true, internetMessageId: true } }); if (!email) throw new HttpError(404, "Email not found");
    const access = await requireMembership(email.workspaceId); assertPermission(access.subject, "EMAIL_TRIAGE"); assertPermission(access.subject, "TASK_CREATE"); const input = await parseJson(request, emailConvertSchema);
    ...
    const result = await prisma.$transaction(async (tx) => {
      if (email.internetMessageId) {
        const existing = await tx.inboundEmail.findFirst({ where: { workspaceId: email.workspaceId, id: { not: emailId }, internetMessageId: { equals: email.internetMessageId, mode: "insensitive" }, taskId: { not: null } }, select: { task: true }, orderBy: { createdAt: "asc" } });
        if (existing?.task) {
