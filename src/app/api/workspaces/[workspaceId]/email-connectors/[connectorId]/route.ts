...
export async function PATCH(request: Request, { params }: { params: Promise<{ workspaceId: string; connectorId: string }> }) {
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      await tx.emailConnector.update({ where: { id: connectorId }, data: { displayName: input.displayName, enabled: input.enabled, syncIntervalMinutes: input.syncIntervalMinutes, ...(input.monitor ? { monitorEnabled: input.monitor.enabled, monitorSlaHours: input.monitor.slaHours, monitorResponderEmails: input.monitor.responderEmails, monitorExcludedSenderEmails: input.monitor.excludedSenderEmails, monitorExcludedSubjectKeywords: input.monitor.excludedSubjectKeywords, monitorSummaryRecipients: input.monitor.summaryRecipients, monitorSummaryEveryHours: input.monitor.summaryEveryHours, monitorLookbackDays: input.monitor.lookbackDays } : {}), configVersion: { increment: 1 } } });
