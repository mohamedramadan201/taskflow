...
export async function GET() { try { const user = await requireUser(); return Response.json(await prisma.workspace.findMany({ where: { members: { some: { userId: user.id, suspendedAt: null } } }, select: { id: true, name: true, slug: true }, orderBy: { name: "asc" } })); } catch (e) { return errorResponse(e); } }
