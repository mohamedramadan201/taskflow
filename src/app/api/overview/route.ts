...
export async function GET(request: Request) { try {
  ...
  const [tasks, members] = await Promise.all([
    prisma.task.findMany({ where: { workspaceId: workspace.id }, include: { assignee: { select: { id: true, name: true, email: true } }, createdBy: { select: { id: true, name: true } } }, orderBy: { createdAt: "desc" }, take: OVERVIEW_TASK_LIMIT }),
