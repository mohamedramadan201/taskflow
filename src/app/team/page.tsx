...
export default async function TeamPage({ searchParams }: { searchParams: Promise<{ workspace?: string }> }) {
  ...
  const [members, invitations, auditEvents, customRoles, teamGroups, workspaces] = await Promise.all([
    ...
    prisma.teamGroup.findMany({ where: { workspaceId: workspace.id }, select: { id: true, name: true, _count: { select: { members: true } } }, orderBy: { name: "asc" } }),
    listUserWorkspaces(session.user.id),
  ]);
  return <AppShell active="team" userName={session.user.name} workspaces={workspaces} workspaceSlug={slug}><TeamClient initialMembers={members} initialInvitations={invitations} initialAuditEvents={auditEvents} initialCustomRoles={customRoles} initialTeamGroups={teamGroups} availablePermissions={customRolePermissions} role={role} currentUserId={session.user.id} workspaceId={workspace.id} /></AppShell>;
}
