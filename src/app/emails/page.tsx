...
export default async function EmailsPage({ searchParams }: { searchParams: Promise<{ workspace?: string }> }) {
  ...
  const [emails, connectors, members, tasks, monitorThreads, linkedThreadTasks] = await Promise.all([
    prisma.inboundEmail.findMany({ where: { workspaceId: access.workspace.id }, include: { connector: { select: { id: true, mailboxAddress: true, displayName: true, monitorEnabled: true } }, task: { select: { id: true, title: true, status: true } }, handledBy: { select: { name: true, email: true } } }, orderBy: { receivedAt: "desc" }, take: 250 }),
    ...
    prisma.emailMonitorThread.findMany({ where: { workspaceId: access.workspace.id }, select: { connectorId: true, gmailThreadId: true, status: true, latestRelevantSenderAddress: true, latestRelevantMessageAt: true, latestExternalMessageAt: true, slaDueAt: true, priority: true, agingBucket: true } }),
    prisma.inboundEmail.findMany({ where: { workspaceId: access.workspace.id, taskId: { not: null } }, select: { connectorId: true, gmailThreadId: true, taskId: true, task: { select: { id: true, title: true, status: true } } } }),
  ]);
